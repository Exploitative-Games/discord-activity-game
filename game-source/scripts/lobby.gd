...
@onready var category_select: Panel = $"Messages/VBoxContainer/Panel/Category Select"
...
func _connect_signals():
	for i in range(1,4):
		category_select.get_child(0).get_child(i).pressed.connect(Callable(self, "_on_category_selected").bind(i))
